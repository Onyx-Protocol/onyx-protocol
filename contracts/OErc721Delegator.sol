...
/**
 * @title Onyx's OErc721Delegator Contract
 * @notice OTokens which wrap an EIP-721 underlying and delegate to an implementation
 * @author Onyx
 */
contract OErc721Delegator is OTokenInterface, OErc721Interface, ODelegatorInterface {
    /**
    ...
    function mints(uint[] calldata tokenIds) external returns (uint[] memory) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("mints(uint256[])", tokenIds));
        return abi.decode(data, (uint[]));
    }
    ...
    function redeems(uint[] calldata redeemTokenIds) external returns (uint[] memory) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("redeems(uint256[])", redeemTokenIds));
        return abi.decode(data, (uint[]));
    }
    ...
    function () external payable {
        require(msg.value == 0,"OErc721Delegator:fallback: cannot send value to fallback");
