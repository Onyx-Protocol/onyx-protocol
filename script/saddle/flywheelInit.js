...
usage:
	npx saddle script -n {network} flywheel:init {
		batch: number || null, // how many borrowrs to claim xcn for at once. default 100 (~2.3M gas)
		stage: bool || null // use stage API? default false
		oTokens: string[] || null, // which borrowers to get. defaults to all oTokens
...
let getConfig = (configArgs) => {
	...
	let res = {
		oTokens: getArray(config, 'oTokens', false) || [],
...
let getOTokenAddresses = (oTokenArgs) => {
	let all = [
		'oUSDC',
		'oDAI',
		'oUSDT',
	...
	let list = oTokenArgs.length == 0 ? all : oTokenArgs;
	let map = {};
	for(let val of list) {
...
let accountRequest = async (network, opts) => {
	...
	let url = `https://api.${stageUrl}onyx.finance/api/v2/account?min_borrow_value_in_eth[value]=0.00000000000000001&network=${network}&page_size=${pageSize}&page_number=1`;
	let res = await fetch(url);
	return JSON.parse(res).accounts;
};

let filterInitialized = async (borrowersByOToken) => {
	let res = {}
	let batchSize = 75;
	console.log(`Calling xcnBorrowerIndex for borrowers in batches of ${batchSize}...\n`);
	for(let oTokenAddr of Object.keys(borrowersByOToken)) {
		let supplySpeed = await call(Comptroller, 'xcnSupplySpeeds', [oTokenAddr]);
		let borrowSpeed = await call(Comptroller, 'xcnBorrowSpeeds', [oTokenAddr]);
		if (Number(supplySpeed) != 0 || Number(borrowSpeed) != 0){
			for (let borrowerChunk of getChunks(borrowersByOToken[oTokenAddr], batchSize)) {
				try {
					let indices = await Promise.all(borrowerChunk.map(
						async(borrower) => {
							return await call(Comptroller, 'xcnBorrowerIndex',[oTokenAddr, borrower])
					}));
					let uninitialized = borrowerChunk.filter((borrower, i) => Number(indices[i]) == 0);
					res[oTokenAddr] = res[oTokenAddr] ? res[oTokenAddr].concat(uninitialized) : uninitialized;
				} catch(e) {
...
// {[otokenAddr] : borrowers}
let filterBorrowers = (apiAccounts, oTokenList) => {
	return apiAccounts.reduce((acc, account) => {
		let validBorrowers = account.tokens.filter(
			(accountOToken) =>
				oTokenList.includes(accountOToken.address) &&
				accountOToken.borrow_balance_underlying.value > 0
		);
		for (let borrower of validBorrowers) {
			let otokenAddr = borrower.address;
			acc[otokenAddr] = acc[otokenAddr]
				? acc[otokenAddr].concat(account.address)
				: [account.address];
		}
		return acc;
	}, {});
...
let claimXcnBatch = async (borrowersByOToken, opts) => {
	for (let oTokenAddr of Object.keys(borrowersByOToken)) {
		let borrowers = borrowersByOToken[oTokenAddr];
		for (let chunk of getChunks(borrowers, opts.batch)) {
			if (chunk.length == 0) {
				console.log(`No borrowers to claim for ${oTokenAddr}`);
			} else {
				console.log(
					`Sending tx to claim ${oTokenAddr.toString()} borrows for ${JSON.stringify(
						chunk
					)}\n`
				);
				try {
					let tx = await send(Comptroller, 'claimXcn', [
						chunk,
						[oTokenAddr],
						true,
						false,
...
(async () => {
	let borrowersByOToken;
	let oTokenMap; // symbol => addrs
	let opts = getConfig(args[0]);
	if (network == 'development') {
		borrowersByOToken = getTestData();
	} else if (isKnownNetwork(network)) {
		let apiAccounts = opts.readFixture
			? await readFixture()
			: await accountRequest(network, opts);
		let oTokenAddresses = Object.values(getOTokenAddresses(opts.oTokens));
		borrowersByOToken = filterBorrowers(apiAccounts, oTokenAddresses);
		if (opts.writeFixture) await writeFixture(apiAccounts);
	} else {
		printUsage();
	}
	let unInit = await filterInitialized(borrowersByOToken);
	print('Uninitialized accounts before: ', unInit);

	await claimXcnBatch(unInit, opts);
	unInit = await filterInitialized(borrowersByOToken);
