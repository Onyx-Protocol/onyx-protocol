...
(async function() {
  ...
  console.log(`Deploying oToken with ${JSON.stringify(conf)}`);
  ...
  let contract = await saddle.deploy('OErc20Delegator', deployArgs);
  ...
  if (env['VERIFY']) {
    ...
    await saddle.verify(etherscanApiKey, contract._address, 'OErc20Delegator', deployArgs, 0);
