...
To run in docker:
    ...
    docker build -t onyx-protocol .

    # Run a shell to the built image
    docker run -it onyx-protocol /bin/sh

From within a docker shell, you can interact locally with the protocol via ganache and truffle:

```bash
    /onyx-protocol > yarn console -n goerli
    Using network goerli https://goerli-eth.onyx.finance
    Saddle console on network goerli https://goerli-eth.onyx.finance
    Deployed goerli contracts
      comptroller: 0x627EA49279FD0dE89186A58b8758aD02B6Be2867
      xcn: 0xfa5E1B628EFB17C024ca76f65B45Faf6B3128CA5
    ...
    > await xcn.methods.totalSupply().call()
...
This command will start a saddle console conencted to Goerli testnet (see [Saddle README](https://github.com/onyx-finance/saddle#cli)):
...
Deploying a OToken from Source
------------------------------
...
Finally, to see if a given deployment matches this version of the Onyx Protocol, you can run `token:match` with a token address and token config:
...
## Deploying a OToken from Docker Build
---------------------------------------

To deploy a specific version of the Onyx Protocol, you can use the `token:deploy` script through Docker:

```bash
docker run --env ETHERSCAN_API_KEY --env VERIFY=true --env ACCOUNT=0x$(cat ~/.ethereum/rinkeby) onyxfinance/onyx-protocol:latest npx saddle -n rinkeby script token:deploy '{
  "underlying": "0x577D296678535e4903D59A4C929B718e1D575e0A",
  "comptroller": "$Comptroller",
  "interestRateModel": "$Base200bps_Slope3000bps",
  "initialExchangeRateMantissa": "2.0e18",
...
To match a deployed contract against a given version of the Onyx Protocol, you can run `token:match` through Docker, passing a token address and config:

```bash
docker run --env ACCOUNT=0x$(cat ~/.ethereum/rinkeby) onyxfinance/onyx-protocol:latest npx saddle -n rinkeby script token:match 0xF1BAd36CB247C82Cb4e9C2874374492Afb50d565 '{
  "underlying": "0x577D296678535e4903D59A4C929B718e1D575e0A",
  "comptroller": "$Comptroller",
  "interestRateModel": "$Base200bps_Slope3000bps",
  "initialExchangeRateMantissa": "2.0e18",
