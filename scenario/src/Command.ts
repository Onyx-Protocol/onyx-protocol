        ...
        if (eventArg === undefined) {
          ...
          try {
            ...
            if (arg.rescue) {
              ...
              // `Gate (OToken Eth Address) (... deploy oToken)`
              // could be used to deploy a oToken if it doesn't exist, but
              // since there is no OToken, that check would raise (when we'd
