...
async function becomeG3(
  ...
  comptrollerImpl: ComptrollerImpl,
  unitroller: Unitroller,
  xcnRate: encodedNumber,
  xcnMarkets: string[],
  otherMarkets: string[]
): Promise<World> {
  let invokation = await invoke(
    world,
    comptrollerImpl.methods._become(unitroller._address, xcnRate, xcnMarkets, otherMarkets),
    from,
    ComptrollerErrorReporter
  );
...
export function comptrollerImplCommands() {
  return [
    ...
    new Command<{
      ...
      comptrollerImpl: ComptrollerImpl;
      xcnRate: NumberV;
      xcnMarkets: ArrayV<AddressV>;
      otherMarkets: ArrayV<AddressV>;
    }>(
      `
        #### BecomeG3

        * "ComptrollerImpl <Impl> BecomeG3 <Rate> <XcnMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl BecomeG3 0.1e18 [oDAI, oETH, oUSDC]
      `,
      'BecomeG3',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
        new Arg('comptrollerImpl', getComptrollerImpl),
        new Arg('xcnRate', getNumberV, { default: new NumberV(1e18) }),
        new Arg('xcnMarkets', getArrayV(getAddressV),  {default: new ArrayV([]) }),
        new Arg('otherMarkets', getArrayV(getAddressV), { default: new ArrayV([]) })
      ],
      (world, from, { unitroller, comptrollerImpl, xcnRate, xcnMarkets, otherMarkets }) => {
        return becomeG3(world, from, comptrollerImpl, unitroller, xcnRate.encode(), xcnMarkets.val.map(a => a.val), otherMarkets.val.map(a => a.val))
      },
      { namePos: 1 }
    ),
    ...
    new Command<{
      ...
      `
        ...
        * "ComptrollerImpl <Impl> Become <Rate> <XcnMarkets> <OtherMarkets>" - Become the comptroller, if possible.
          * E.g. "ComptrollerImpl MyImpl Become 0.1e18 [oDAI, oETH, oUSDC]
      `,
      'Become',
      [
        new Arg('unitroller', getUnitroller, { implicit: true }),
