...
async function setPrice(world: World, from: string, priceOracle: PriceOracle, oToken: string, amount: NumberV): Promise<World> {
  return addAction(
    world,
    `Set price oracle price for ${oToken} to ${amount.show()}`,
    await invoke(world, priceOracle.methods.setUnderlyingPrice(oToken, amount.encode()), from)
  );
}
...
export function priceOracleCommands() {
  return [
    ...
    new Command<{priceOracle: PriceOracle, oToken: AddressV, amount: NumberV}>(`
        #### SetPrice
      ...
      [
        ...
        new Arg("oToken", getAddressV),
        new Arg("amount", getExpNumberV)
      ],
      (world, from, {priceOracle, oToken, amount}) => setPrice(world, from, priceOracle, oToken.val, amount)
    ),

    new Command<{priceOracle: PriceOracle, address: AddressV, amount: NumberV}>(`
        #### SetDirectPrice

        * "SetDirectPrice <Address> <Amount>" - Sets the per-ether price for the given oToken
