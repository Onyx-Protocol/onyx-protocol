...
async function supportMarket(world: World, from: string, comptroller: Comptroller, oToken: OToken): Promise<World> {
  if (world.dryRun) {
    // Skip this specifically on dry runs since it's likely to crash due to a number of reasons
    world.printer.printLine(`Dry run: Supporting market  \`${oToken._address}\``);
    return world;
  }

  let invokation = await invoke(world, comptroller.methods._supportMarket(oToken._address), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Supported market ${oToken.name}`,
    invokation
  );
...
async function unlistMarket(world: World, from: string, comptroller: Comptroller, oToken: OToken): Promise<World> {
  let invokation = await invoke(world, comptroller.methods.unlist(oToken._address), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Unlisted market ${oToken.name}`,
    invokation
  );
...
async function setCollateralFactor(world: World, from: string, comptroller: Comptroller, oToken: OToken, collateralFactor: NumberV): Promise<World> {
  let invokation = await invoke(world, comptroller.methods._setCollateralFactor(oToken._address, collateralFactor.encode()), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Set collateral factor for ${oToken.name} to ${collateralFactor.show()}`,
    invokation
  );
...
async function addXcnMarkets(world: World, from: string, comptroller: Comptroller, oTokens: OToken[]): Promise<World> {
  let invokation = await invoke(world, comptroller.methods._addXcnMarkets(oTokens.map(c => c._address)), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Added XCN markets ${oTokens.map(c => c.name)}`,
    invokation
  );
...
async function dropXcnMarket(world: World, from: string, comptroller: Comptroller, oToken: OToken): Promise<World> {
  let invokation = await invoke(world, comptroller.methods._dropXcnMarket(oToken._address), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Drop XCN market ${oToken.name}`,
    invokation
  );
...
async function refreshXcnSpeeds(world: World, from: string, comptroller: Comptroller): Promise<World> {
  let invokation = await invoke(world, comptroller.methods.refreshXcnSpeeds(), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Refreshed XCN speeds`,
    invokation
  );
...
async function claimXcn(world: World, from: string, comptroller: Comptroller, holder: string): Promise<World> {
  let invokation = await invoke(world, comptroller.methods.claimXcn(holder), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Xcn claimed by ${holder}`,
    invokation
  );
...
async function claimXcnInMarkets(world: World, from: string, comptroller: Comptroller, holder: string, oTokens: OToken[]): Promise<World> {
  let invokation = await invoke(world, comptroller.methods.claimXcn(holder, oTokens.map(c => c._address)), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Xcn claimed by ${holder} in markets ${oTokens.map(c => c.name)}`,
    invokation
  );
...
async function grantXcn(world: World, from: string, comptroller: Comptroller, recipient: string, amount: NumberV): Promise<World> {
  let invokation = await invoke(world, comptroller.methods._grantXcn(recipient, amount.encode()), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `${amount.show()} xcn granted to ${recipient}`,
    invokation
  );
...
async function setXcnRate(world: World, from: string, comptroller: Comptroller, rate: NumberV): Promise<World> {
  let invokation = await invoke(world, comptroller.methods._setXcnRate(rate.encode()), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Xcn rate set to ${rate.show()}`,
    invokation
  );
...
async function setXcnSpeed(world: World, from: string, comptroller: Comptroller, oToken: OToken, speed: NumberV): Promise<World> {
  let invokation = await invoke(world, comptroller.methods._setXcnSpeed(oToken._address, speed.encode()), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Xcn speed for market ${oToken._address} set to ${speed.show()}`,
    invokation
  );
...
async function setXcnSpeeds(world: World, from: string, comptroller: Comptroller, oTokens: OToken[], supplySpeeds: NumberV[], borrowSpeeds: NumberV[]): Promise<World> {
  let invokation = await invoke(world, comptroller.methods._setXcnSpeeds(oTokens.map(c => c._address), supplySpeeds.map(speed => speed.encode()), borrowSpeeds.map(speed => speed.encode())), from, ComptrollerErrorReporter);

  world = addAction(
    world,
    `Xcn speed for markets [${oTokens.map(c => c._address)}] set to supplySpeeds=[${supplySpeeds.map(speed => speed.show())}, borrowSpeeds=[${borrowSpeeds.map(speed => speed.show())}]`,
    invokation
  );
...
async function setGuardianMarketPaused(world: World, from: string, comptroller: Comptroller, oToken: OToken, action: string, state: boolean): Promise<World> {
  let fun;
  switch(action){
    case "Mint":
  ...
  let invokation = await invoke(world, fun(oToken._address, state), from, ComptrollerErrorReporter);
...
async function setMarketBorrowCaps(world: World, from: string, comptroller: Comptroller, oTokens: OToken[], borrowCaps: NumberV[]): Promise<World> {
  ...
  world = addAction(
    world,
    `Borrow caps on ${oTokens} set to ${borrowCaps}`,
    invokation
  );
...
export function comptrollerCommands() {
  return [
    ...
    new Command<{comptroller: Comptroller, action: StringV, isPaused: BoolV}>(`
        ...
        * "Comptroller SetPaused <Action> <Bool>" - Pauses or unpaused given oToken function
    ...
    new Command<{comptroller: Comptroller, oToken: OToken}>(`
        #### SupportMarket
      ...
      (world, from, {comptroller, oToken}) => supportMarket(world, from, comptroller, oToken)
    ),
    new Command<{comptroller: Comptroller, oToken: OToken}>(`
        #### UnList

        * "Comptroller UnList <OToken>" - Mock unlists a given market in tests
          * E.g. "Comptroller UnList oZRX"
      `,
      "UnList",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, oToken}) => unlistMarket(world, from, comptroller, oToken)
    ),
    new Command<{comptroller: Comptroller, oTokens: OToken[]}>(`
        #### EnterMarkets

        * "Comptroller EnterMarkets (<OToken> ...)" - User enters the given markets
          * E.g. "Comptroller EnterMarkets (oZRX oETH)"
      `,
      "EnterMarkets",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, oTokens}) => enterMarkets(world, from, comptroller, oTokens.map((c) => c._address))
    ),
    new Command<{comptroller: Comptroller, oToken: OToken}>(`
        #### ExitMarket

        * "Comptroller ExitMarket <OToken>" - User exits the given markets
          * E.g. "Comptroller ExitMarket oZRX"
      `,
      "ExitMarket",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, oToken}) => exitMarket(world, from, comptroller, oToken._address)
    ),
    ...
    new Command<{comptroller: Comptroller, oToken: OToken, collateralFactor: NumberV}>(`
        #### SetCollateralFactor
      ...
      (world, from, {comptroller, oToken, collateralFactor}) => setCollateralFactor(world, from, comptroller, oToken, collateralFactor)
    ),
    ...
    new Command<{comptroller: Comptroller, action: StringV, isPaused: BoolV}>(`
        ...
        * "Comptroller SetGuardianPaused <Action> <Bool>" - Pauses or unpaused given oToken function
    ...
    new Command<{comptroller: Comptroller, oToken: OToken, action: StringV, isPaused: BoolV}>(`
        #### SetGuardianMarketPaused

        * "Comptroller SetGuardianMarketPaused <OToken> <Action> <Bool>" - Pauses or unpaused given oToken function
        * E.g. "Comptroller SetGuardianMarketPaused oREP "Mint" True"
        `,
        "SetGuardianMarketPaused",
        [
          new Arg("comptroller", getComptroller, {implicit: true}),
        ...
        (world, from, {comptroller, oToken, action, isPaused}) => setGuardianMarketPaused(world, from, comptroller, oToken, action.val, isPaused.val)
    ),
    ...
    new Command<{comptroller: Comptroller, signature: StringV, callArgs: StringV[]}>(`
      ...
      * E.g: Comptroller Send "setXcnAddress(address)" (Address XCN)
      `,
      "Send",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
    ...
    new Command<{comptroller: Comptroller, oTokens: OToken[]}>(`
      ...
      * "Comptroller AddXcnMarkets (<Address> ...)" - Makes a market XCN-enabled
      * E.g. "Comptroller AddXcnMarkets (oZRX oBAT)
      `,
      "AddXcnMarkets",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, oTokens}) => addXcnMarkets(world, from, comptroller, oTokens)
     ),
    new Command<{comptroller: Comptroller, oToken: OToken}>(`
      #### DropXcnMarket

      * "Comptroller DropXcnMarket <Address>" - Makes a market XCN
      * E.g. "Comptroller DropXcnMarket oZRX
      `,
      "DropXcnMarket",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, oToken}) => dropXcnMarket(world, from, comptroller, oToken)
     ),

    new Command<{comptroller: Comptroller}>(`
      #### RefreshXcnSpeeds

      * "Comptroller RefreshXcnSpeeds" - Recalculates all the XCN market speeds
      * E.g. "Comptroller RefreshXcnSpeeds
      `,
      "RefreshXcnSpeeds",
      [
        new Arg("comptroller", getComptroller, {implicit: true})
      ],
      (world, from, {comptroller}) => refreshXcnSpeeds(world, from, comptroller)
    ),
    new Command<{comptroller: Comptroller, holder: AddressV}>(`
      #### ClaimXcn

      * "Comptroller ClaimXcn <holder>" - Claims xcn
      * E.g. "Comptroller ClaimXcn Geoff
      `,
      "ClaimXcn",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, holder}) => claimXcn(world, from, comptroller, holder.val)
    ),
    new Command<{comptroller: Comptroller, holder: AddressV, oTokens: OToken[]}>(`
      #### ClaimXcnInMarkets

      * "Comptroller ClaimXcn <holder> (<OToken> ...)" - Claims xcn
      * E.g. "Comptroller ClaimXcnInMarkets Geoff (oDAI oBAT)
      `,
      "ClaimXcnInMarkets",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, holder, oTokens}) => claimXcnInMarkets(world, from, comptroller, holder.val, oTokens)
    ),
    ...
    new Command<{comptroller: Comptroller, recipient: AddressV, amount: NumberV}>(`
      ...
      * "Comptroller GrantComp <recipient> <amount>" - Grants XCN to a recipient
      * E.g. "Comptroller GrantComp Geoff 1e18
      ...
      (world, from, {comptroller, recipient, amount}) => grantXcn(world, from, comptroller, recipient.val, amount)
    ),
    new Command<{comptroller: Comptroller, rate: NumberV}>(`
      #### SetXcnRate

      * "Comptroller SetXcnRate <rate>" - Sets XCN rate
      * E.g. "Comptroller SetXcnRate 1e18
      `,
      "SetXcnRate",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, rate}) => setXcnRate(world, from, comptroller, rate)
    ),
    new Command<{comptroller: Comptroller, oToken: OToken, speed: NumberV}>(`
      #### SetXcnSpeed (deprecated)
      * "Comptroller SetXcnSpeed <oToken> <rate>" - Sets XCN speed for market
      * E.g. "Comptroller SetXcnSpeed oToken 1000
      `,
      "SetXcnSpeed",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
      ...
      (world, from, {comptroller, oToken, speed}) => setXcnSpeed(world, from, comptroller, oToken, speed)
    ),
    new Command<{comptroller: Comptroller, oTokens: OToken[], supplySpeeds: NumberV[], borrowSpeeds: NumberV[]}>(`
      #### SetXcnSpeeds
      * "Comptroller SetXcnSpeeds (<oToken> ...) (<supplySpeed> ...) (<borrowSpeed> ...)" - Sets XCN speeds for markets
      * E.g. "Comptroller SetXcnSpeeds (oZRX oBAT) (1000 0) (1000 2000)
      `,
      "SetXcnSpeeds",
      [
        new Arg("comptroller", getComptroller, {implicit: true}),
