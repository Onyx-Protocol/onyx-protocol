...
import { getXcnValue, xcnFetchers } from './Value/XcnValue';
...
const fetchers = [
    ...
    async (world, { addr, slot, key, nestedKey, valType }) => {
      ...
      switch (valType.val) {
        case 'marketStruct':
          ...
          let isXcnKey = '0x' + toBN(newKey).add(toBN(3)).toString(16);
          let isXcnStr = await world.web3.eth.getStorageAt(addr.val, isXcnKey);

          return new ListV([
            new BoolV(isListed),
            new ExpNumberV(collateralFactor.toString(), 1e18),
            new BoolV(areEqual(userInMarket, 1)),
            new BoolV(areEqual(isXcnStr, 1))
          ]);
        default:
          return new NothingV();
      }
  ...
  new Fetcher<{ given: Value; expected: Value }, BoolV>(
    `
      ...
      * "Equal given:<Value> expected:<Value>" - Returns true if given values are equal
        ...
        * E.g. "Equal (OToken oZRX TotalSupply) (Exactly 55)"
        * E.g. "Equal (OToken oZRX Comptroller) (Comptroller Address)"
    `,
    'Equal',
  ...
  new Fetcher<{ res: Value }, Value>(
    ...
    `,
    'OToken',
    [new Arg('res', getOTokenValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: oTokenFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### OTokenDelegate

      * "OTokenDelegate ...oTokenDelegateArgs" - Returns oToken delegate value
    `,
    'OTokenDelegate',
    [new Arg('res', getOTokenDelegateValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: oTokenDelegateFetchers() }
  ),
  new Fetcher<{ res: Value }, Value>(
    `
      #### Erc20
  ...
  new Fetcher<{ res: Value }, Value>(
    `
      ...
      * "Xcn ...xcnArgs" - Returns Xcn value
    `,
    'Xcn',
    [new Arg('res', getXcnValue, { variadic: true })],
    async (world, { res }) => res,
    { subExpressions: xcnFetchers() }
  ),
];

let contractFetchers = [
  { contract: "Counter", implicit: false },
  { contract: "OnyxLens", implicit: false },
