...
describe('Flywheel', () => {
  ...
  describe('updateContributorRewards', () => {
    ...
    it('should accrue xcn to contributors', async () => {
      const tx1 = await send(comptroller, '_setContributorXcnSpeed', [a1, 2000]);
      await fastForward(comptroller, 50);

      const a1Accrued = await xcnAccrued(comptroller, a1);
      expect(a1Accrued).toEqualNumber(0);

      const tx2 = await send(comptroller, 'updateContributorRewards', [a1], {from: a1});
      const a1Accrued2 = await xcnAccrued(comptroller, a1);
      expect(a1Accrued2).toEqualNumber(50 * 2000);
    });

    it('should accrue xcn with late set', async () => {
  ...
  describe('_setContributorXcnSpeed', () => {
    it('should revert if not called by admin', async () => {
      await expect(
        send(comptroller, '_setContributorXcnSpeed', [a1, 1000], {from: a1})
      ).rejects.toRevert('revert only admin can set xcn speed');
    });

    it('should start xcn stream if called by admin', async () => {
      const tx = await send(comptroller, '_setContributorXcnSpeed', [a1, 1000]);
      expect(tx).toHaveLog('ContributorXcnSpeedUpdated', {
        contributor: a1,
        newSpeed: 1000
      });
    });

    it('should reset xcn stream if set to 0', async () => {
      const tx1 = await send(comptroller, '_setContributorXcnSpeed', [a1, 2000]);
      await fastForward(comptroller, 50);

      const tx2 = await send(comptroller, '_setContributorXcnSpeed', [a1, 0]);
      await fastForward(comptroller, 50);
