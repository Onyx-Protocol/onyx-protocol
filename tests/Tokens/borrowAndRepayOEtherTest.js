...
async function preBorrow(oToken, borrower, borrowAmount) {
  ...
  await setEtherBalance(oToken, borrowAmount);
}

async function borrowFresh(oToken, borrower, borrowAmount) {
  return send(oToken, 'harnessBorrowFresh', [borrower, borrowAmount], {from: borrower});
}
...
async function repayBorrowFresh(oToken, payer, borrower, repayAmount) {
  return send(oToken, 'harnessRepayBorrowFresh', [payer, borrower, repayAmount], {from: payer, value: repayAmount});
}

async function repayBorrow(oToken, borrower, repayAmount) {
  await send(oToken, 'harnessFastForward', [1]);
  return send(oToken, 'repayBorrow', [], {from: borrower, value: repayAmount});
}

async function repayBorrowBehalf(oToken, payer, borrower, repayAmount) {
  await send(oToken, 'harnessFastForward', [1]);
  return send(oToken, 'repayBorrowBehalf', [borrower], {from: payer, value: repayAmount});
}
...
describe('OEther', function () {
  ...
  describe('borrowFresh', () => {
    ...
    it("transfers the underlying cash, tokens, and emits Borrow event", async () => {
      const beforeBalances = await getBalances([oToken], [borrower]);
      const beforeProtocolBorrows = await totalBorrows(oToken);
      const result = await borrowFresh(oToken, borrower, borrowAmount);
      const afterBalances = await getBalances([oToken], [borrower]);
      expect(result).toSucceed();
      expect(afterBalances).toEqual(await adjustBalances(beforeBalances, [
        [oToken, 'eth', -borrowAmount],
        [oToken, 'borrows', borrowAmount],
        [oToken, borrower, 'eth', borrowAmount.minus(await etherGasCost(result))],
        [oToken, borrower, 'borrows', borrowAmount]
      ]));
  ...
  describe('borrow', () => {
    ...
    it("returns success from borrowFresh and transfers the correct amount", async () => {
      const beforeBalances = await getBalances([oToken], [borrower]);
      await fastForward(oToken);
      const result = await borrow(oToken, borrower, borrowAmount);
      const afterBalances = await getBalances([oToken], [borrower]);
      expect(result).toSucceed();
      expect(afterBalances).toEqual(await adjustBalances(beforeBalances, [
        [oToken, 'eth', -borrowAmount],
    ...
    [true, false].forEach(async (benefactorPaying) => {
      ...
      describe(label, () => {
        ...
        it("reverts if checkTransferIn fails", async () => {
          await expect(
            send(oToken, 'harnessRepayBorrowFresh', [payer, borrower, repayAmount], {from: root, value: repayAmount})
          ).rejects.toRevert("revert sender mismatch");
          await expect(
            send(oToken, 'harnessRepayBorrowFresh', [payer, borrower, repayAmount], {from: payer, value: 1})
        ...
        it("transfers the underlying cash, and emits RepayBorrow event", async () => {
          ...
          if (borrower == payer) {
            expect(afterBalances).toEqual(await adjustBalances(beforeBalances, [
              [oToken, 'eth', repayAmount],
              [oToken, 'borrows', -repayAmount],
              [oToken, borrower, 'borrows', -repayAmount],
              [oToken, borrower, 'eth', -repayAmount.plus(await etherGasCost(result))]
            ]));
          } else {
            expect(afterBalances).toEqual(await adjustBalances(beforeBalances, [
              [oToken, 'eth', repayAmount],
  ...
  describe('repayBorrow', () => {
    ...
    it("reverts when repay borrow fresh fails", async () => {
      await send(oToken.comptroller, 'setRepayBorrowAllowed', [false]);
      await expect(repayBorrow(oToken, borrower, repayAmount)).rejects.toRevertWithError('COMPTROLLER_REJECTION', "revert repayBorrow failed");
    });
    ...
    it("reverts if overpaying", async () => {
      ...
      await expect(repayBorrow(oToken, borrower, tooMuch)).rejects.toRevert("revert REPAY_BORROW_NEW_ACCOUNT_BORROW_BALANCE_CALCULATION_FAILED");
      // await assert.toRevertWithError(repayBorrow(oToken, borrower, tooMuch), 'MATH_ERROR', "revert repayBorrow failed");
    });
  });
  ...
  describe('repayBorrowBehalf', () => {
    ...
    it("reverts from within repay borrow fresh", async () => {
      await send(oToken.comptroller, 'setRepayBorrowAllowed', [false]);
      await expect(repayBorrowBehalf(oToken, payer, borrower, repayAmount)).rejects.toRevertWithError('COMPTROLLER_REJECTION', "revert repayBorrowBehalf failed");
    });
