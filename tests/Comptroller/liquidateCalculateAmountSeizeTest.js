...
async function calculateSeizeTokens(comptroller, oTokenBorrowed, oTokenCollateral, repayAmount) {
  return call(comptroller, 'liquidateCalculateSeizeTokens', [oTokenBorrowed._address, oTokenCollateral._address, repayAmount]);
}
...
describe('Comptroller', () => {
  let root, accounts;
  let comptroller, oTokenBorrowed, oTokenCollateral;

  beforeEach(async () => {
    [root, ...accounts] = saddle.accounts;
    comptroller = await makeComptroller();
    oTokenBorrowed = await makeOToken({comptroller: comptroller, underlyingPrice: 0});
    oTokenCollateral = await makeOToken({comptroller: comptroller, underlyingPrice: 0});
  });

  beforeEach(async () => {
    await setOraclePrice(oTokenBorrowed, borrowedPrice);
    await setOraclePrice(oTokenCollateral, collateralPrice);
    await send(oTokenCollateral, 'harnessExchangeRateDetails', [8e10, 4e10, 0]);
  });

  describe('liquidateCalculateAmountSeize', () => {
    it("fails if either asset price is 0", async () => {
      await setOraclePrice(oTokenBorrowed, 0);
      expect(
        await calculateSeizeTokens(comptroller, oTokenBorrowed, oTokenCollateral, repayAmount)
      ).toHaveTrollErrorTuple(['PRICE_ERROR', 0]);

      await setOraclePrice(oTokenCollateral, 0);
      expect(
        await calculateSeizeTokens(comptroller, oTokenBorrowed, oTokenCollateral, repayAmount)
      ).toHaveTrollErrorTuple(['PRICE_ERROR', 0]);
    });
    ...
    it("fails if the repayAmount causes overflow ", async () => {
      await expect(
        calculateSeizeTokens(comptroller, oTokenBorrowed, oTokenCollateral, UInt256Max())
      ).rejects.toRevert("revert multiplication overflow");
    });

    it("fails if the borrowed asset price causes overflow ", async () => {
      await setOraclePrice(oTokenBorrowed, -1);
      await expect(
        calculateSeizeTokens(comptroller, oTokenBorrowed, oTokenCollateral, repayAmount)
      ).rejects.toRevert("revert multiplication overflow");
    });

    it("reverts if it fails to calculate the exchange rate", async () => {
      await send(oTokenCollateral, 'harnessExchangeRateDetails', [1, 0, 10]); // (1 - 10) -> underflow
      await expect(
        send(comptroller, 'liquidateCalculateSeizeTokens', [oTokenBorrowed._address, oTokenCollateral._address, repayAmount])
    ...
    [
      ...
      it(`returns the correct value for ${testCase}`, async () => {
        ...
        await send(oTokenCollateral, 'harnessSetExchangeRate', [exchangeRate]);
