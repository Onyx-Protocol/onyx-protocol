...
describe('Comptroller', () => {
  ...
  describe('_setCloseFactor', () => {
    it("fails if not called by admin", async () => {
      ...
      await expect(
        send(oToken.comptroller, '_setCloseFactor', [1], {from: accounts[0]})
  ...
  describe('_setCollateralFactor', () => {
    ...
    it("fails if not called by admin", async () => {
      const oToken = await makeOToken();
      expect(
        await send(oToken.comptroller, '_setCollateralFactor', [oToken._address, half], {from: accounts[0]})
    ...
    it("fails if asset is not listed", async () => {
      const oToken = await makeOToken();
      expect(
        await send(oToken.comptroller, '_setCollateralFactor', [oToken._address, half])
    ...
    it("succeeds and sets market", async () => {
      const oToken = await makeOToken({supportMarket: true, underlyingPrice: 1});
      const result = await send(oToken.comptroller, '_setCollateralFactor', [oToken._address, half]);
  ...
  describe('_supportMarket', () => {
    it("fails if not called by admin", async () => {
      const oToken = await makeOToken(root);
      expect(
        await send(oToken.comptroller, '_supportMarket', [oToken._address], {from: accounts[0]})
    ...
    it("succeeds and sets market", async () => {
      const oToken = await makeOToken();
      const result = await send(oToken.comptroller, '_supportMarket', [oToken._address]);
      expect(result).toHaveLog('MarketListed', {oToken: oToken._address});
    });

    it("cannot list a market a second time", async () => {
      const oToken = await makeOToken();
      const result1 = await send(oToken.comptroller, '_supportMarket', [oToken._address]);
      const result2 = await send(oToken.comptroller, '_supportMarket', [oToken._address]);
    ...
    it("can list two different markets", async () => {
      const oToken1 = await makeOToken();
      const oToken2 = await makeOToken({comptroller: oToken1.comptroller});
      const result1 = await send(oToken1.comptroller, '_supportMarket', [oToken1._address]);
      const result2 = await send(oToken1.comptroller, '_supportMarket', [oToken2._address]);
  ...
  describe('redeemVerify', () => {
    it('should allow you to redeem 0 underlying for 0 tokens', async () => {
      const comptroller = await makeComptroller();
      const oToken = await makeOToken({comptroller: comptroller});
      await call(comptroller, 'redeemVerify', [oToken._address, accounts[0], 0, 0]);
    });

    it('should allow you to redeem 5 underlyig for 5 tokens', async () => {
      const comptroller = await makeComptroller();
      const oToken = await makeOToken({comptroller: comptroller});
      await call(comptroller, 'redeemVerify', [oToken._address, accounts[0], 5, 5]);
    });

    it('should not allow you to redeem 5 underlying for 0 tokens', async () => {
      const comptroller = await makeComptroller();
      const oToken = await makeOToken({comptroller: comptroller});
      await expect(call(comptroller, 'redeemVerify', [oToken._address, accounts[0], 5, 0])).rejects.toRevert("revert redeemTokens zero");
    });
  })
});
