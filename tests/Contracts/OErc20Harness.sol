...
import "../../contracts/OErc20Immutable.sol";
import "../../contracts/OErc20Delegator.sol";
import "../../contracts/OErc20Delegate.sol";
import "../../contracts/ODaiDelegate.sol";
import "./ComptrollerScenario.sol";

contract OErc20Harness is OErc20Immutable {
    ...
    function harnessRedeemFresh(address payable account, uint oTokenAmount, uint underlyingAmount) public returns (uint) {
        return super.redeemFresh(account, oTokenAmount, underlyingAmount);
    }
    ...
    function harnessLiquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, OToken oTokenCollateral) public returns (uint) {
        (uint err,) = liquidateBorrowFresh(liquidator, borrower, repayAmount, oTokenCollateral);
        return err;
    }
...
contract OErc20Scenario is OErc20Immutable {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
...
contract CEvil is OErc20Scenario {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
    ...
    function evilSeize(OToken treasure, address liquidator, address borrower, uint seizeTokens) public returns (uint) {
        return treasure.seize(liquidator, borrower, seizeTokens);
    }
}

contract OErc20DelegatorScenario is OErc20Delegator {
    constructor(address underlying_,
                ComptrollerInterface comptroller_,
                InterestRateModel interestRateModel_,
                uint initialExchangeRateMantissa_,
...
contract OErc20DelegateHarness is OErc20Delegate {
...
contract OErc20DelegateScenario is OErc20Delegate {
    constructor() public {}
...
contract OErc20DelegateScenarioExtra is OErc20DelegateScenario {
...
contract ODaiDelegateHarness is ODaiDelegate {
    uint blockNumber = 100000;
    uint harnessExchangeRate;
    bool harnessExchangeRateStored;
...
contract ODaiDelegateScenario is ODaiDelegate {
    function setTotalBorrows(uint totalBorrows_) public {
        totalBorrows = totalBorrows_;
    }
...
contract ODaiDelegateMakerHarness is PotLike, VatLike, GemLike, DaiJoinLike {
    /* Pot */
