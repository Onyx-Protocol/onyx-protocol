...
contract ERC721Harness is Context, ERC165, IERC721 {
    using SafeMath for uint256;
    ...
    function transferFrom(address from, address to, uint256 tokenId) public {
        ...
        require(!failTransferFromAddresses[from], "ERC721: TOKEN_TRANSFER_FAILED");
        require(!failTransferToAddresses[to], "ERC721: TOKEN_TRANSFER_FAILED");
    ...
    function harnessSetTotalSupply(uint256 value) public {
        _totalSupply = value;
    }
    ...
    function harnessMint(address to, uint256 tokenId) public {
        _mint(to, tokenId);
    }

    function harnessSetApprovalForAll(address owner, address to, bool approved) public {
        require(to != owner, "ERC721: approve to caller");

        _operatorApprovals[owner][to] = approved;
        emit ApprovalForAll(owner, to, approved);
    }

    function harnessSetBalance(address _account, uint _amount) public {
        _ownedTokensCount[_account]._value = _amount;
    }
}
